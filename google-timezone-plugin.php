...
class Plugin {
	...
	public function add_menu_page(): void {
		...
		if ( ! isset( $admin_page_hooks['arraypress-google'] ) ) {
			add_menu_page(
				__( 'Google', 'arraypress-google-timezone' ),
				__( 'Google', 'arraypress-google-timezone' ),
				'manage_options',
				'arraypress-google',
				null,
		...
		add_submenu_page(
			'arraypress-google',
			__( 'Timezone Detection', 'arraypress-google-timezone' ),
			__( 'Timezone Detection', 'arraypress-google-timezone' ),
			'manage_options',
			'arraypress-google-timezone',
	...
	private function process_form_submissions(): array {
		...
		if ( isset( $_POST['clear_cache'] ) ) {
			...
			add_settings_error(
				'timezone_test',
				'cache_cleared',
				__( 'Cache cleared successfully', 'arraypress-google-timezone' ),
				'success'
			);
		}
	...
	private function render_timezone_details( $result ): void {
		...
		<table class="widefat striped">
			<tbody>
			<tr>
				<th><?php _e( 'Timezone ID', 'arraypress-google-timezone' ); ?></th>
				<td><?php echo esc_html( $result->get_timezone_id() ); ?></td>
			</tr>
			<tr>
				<th><?php _e( 'Timezone Name', 'arraypress-google-timezone' ); ?></th>
				<td><?php echo esc_html( $result->get_timezone_name() ); ?></td>
			</tr>
			<tr>
				<th><?php _e( 'Abbreviated Name', 'arraypress-google-timezone' ); ?></th>
				<td><?php echo esc_html( $result->get_abbreviated_name() ); ?></td>
			</tr>
			<tr>
				<th><?php _e( 'Raw UTC Offset', 'arraypress-google-timezone' ); ?></th>
				<td><?php echo esc_html( $result->get_raw_offset() . ' seconds' ); ?></td>
			</tr>
			<tr>
				<th><?php _e( 'DST Offset', 'arraypress-google-timezone' ); ?></th>
				<td><?php echo esc_html( $result->get_dst_offset() . ' seconds' ); ?></td>
			</tr>
			<tr>
				<th><?php _e( 'Total Offset', 'arraypress-google-timezone' ); ?></th>
				<td><?php echo esc_html( $result->get_total_offset() . ' seconds' ); ?></td>
			</tr>
			<tr>
				<th><?php _e( 'Formatted Offset', 'arraypress-google-timezone' ); ?></th>
				<td><?php echo esc_html( $result->get_formatted_offset() ); ?></td>
			</tr>
			<tr>
				<th><?php _e( 'Is DST Active', 'arraypress-google-timezone' ); ?></th>
				<td><?php echo $result->is_dst() ? 'Yes' : 'No'; ?></td>
			</tr>
			<tr>
				<th><?php _e( 'Local Date/Time', 'arraypress-google-timezone' ); ?></th>
				<td>
					<?php
	...
	public function render_test_page(): void {
		...
		<div class="wrap timezone-test">
			<h1><?php _e( 'Google Maps Timezone API Test', 'arraypress-google-timezone' ); ?></h1>
			...
			<?php if ( empty( get_option( 'google_timezone_api_key' ) ) ): ?>
				...
				<div class="notice notice-warning">
					<p><?php _e( 'Please enter your Google Maps API key to begin testing.', 'arraypress-google-timezone' ); ?></p>
				</div>
			...
			<?php else: ?>
				...
				<div class="timezone-test-container">
					...
					<div class="timezone-test-section">
						<h2><?php _e( 'Timezone Detection', 'arraypress-google-timezone' ); ?></h2>
						<form method="post" class="timezone-form">
							<?php wp_nonce_field( 'timezone_test' ); ?>
							<table class="form-table">
								<tr>
									<th scope="row">
										<label for="latitude"><?php _e( 'Latitude', 'arraypress-google-timezone' ); ?></label>
									</th>
									<td>
										<input type="number" name="latitude" id="latitude" class="regular-text"
										       value="37.4224764" step="any"
										       placeholder="<?php esc_attr_e( 'Enter latitude...', 'arraypress-google-timezone' ); ?>">
									</td>
								</tr>
								<tr>
									<th scope="row">
										<label for="longitude"><?php _e( 'Longitude', 'arraypress-google-timezone' ); ?></label>
									</th>
									<td>
										<input type="number" name="longitude" id="longitude" class="regular-text"
										       value="-122.0842499" step="any"
										       placeholder="<?php esc_attr_e( 'Enter longitude...', 'arraypress-google-timezone' ); ?>">
									</td>
								</tr>
								<tr>
									<th scope="row">
										<label for="timestamp"><?php _e( 'Date/Time', 'arraypress-google-timezone' ); ?></label>
									</th>
									<td>
										<input type="datetime-local" name="timestamp" id="timestamp" class="regular-text">
										<p class="description">
											<?php _e( 'Optional. Leave blank for current time.', 'arraypress-google-timezone' ); ?>
										</p>
									</td>
								</tr>
								<tr>
									<th scope="row">
										<label for="language_code"><?php _e( 'Language', 'arraypress-google-timezone' ); ?></label>
									</th>
									<td>
										<select name="language_code" id="language_code" class="regular-text">
											<option value=""><?php _e( 'Default', 'arraypress-google-timezone' ); ?></option>
										...
										<p class="description">
											<?php _e( 'Select preferred language for timezone names', 'arraypress-google-timezone' ); ?>
										</p>
									</td>
								</tr>
							</table>
							<?php submit_button( __( 'Get Timezone', 'arraypress-google-timezone' ), 'primary', 'submit_timezone' ); ?>
						</form>

						<?php if ( $results['timezone'] ): ?>
							<h3><?php _e( 'Timezone Results', 'arraypress-google-timezone' ); ?></h3>
				...
				<!-- Cache Management -->
				<div class="timezone-test-section">
					<h2><?php _e( 'Cache Management', 'arraypress-google-timezone' ); ?></h2>
					<form method="post" class="timezone-form">
						<?php wp_nonce_field( 'timezone_test' ); ?>
						<p class="description">
							<?php _e( 'Clear the cached timezone results. This will force new API requests for subsequent lookups.', 'arraypress-google-timezone' ); ?>
						</p>
						<?php submit_button( __( 'Clear Cache', 'arraypress-google-timezone' ), 'delete', 'clear_cache' ); ?>
					</form>
				</div>
	...
	private function render_settings_form(): void {
		?>
		<h2><?php _e( 'Settings', 'arraypress-google-timezone' ); ?></h2>
		<form method="post" class="timezone-form">
			<?php wp_nonce_field( 'timezone_api_key' ); ?>
			<table class="form-table">
				<tr>
					<th scope="row">
						<label for="google_timezone_api_key"><?php _e( 'API Key', 'arraypress-google-timezone' ); ?></label>
					</th>
					<td>
						<input type="text" name="google_timezone_api_key" id="google_timezone_api_key"
						       ...
						       placeholder="<?php esc_attr_e( 'Enter your Google Maps API key...', 'arraypress-google-timezone' ); ?>">
						<p class="description">
							<?php _e( 'Your Google Maps API key. Required for making API requests.', 'arraypress-google-timezone' ); ?>
						</p>
					</td>
				</tr>
				<tr>
					<th scope="row">
						<label for="google_timezone_enable_cache"><?php _e( 'Enable Cache', 'arraypress-google-timezone' ); ?></label>
					</th>
					<td>
						<label>
							...
							<?php _e( 'Cache timezone results', 'arraypress-google-timezone' ); ?>
						</label>
						<p class="description">
							<?php _e( 'Caching results can help reduce API usage and improve performance.', 'arraypress-google-timezone' ); ?>
						</p>
					</td>
				</tr>
				<tr>
					<th scope="row">
						<label for="google_timezone_cache_duration"><?php _e( 'Cache Duration', 'arraypress-google-timezone' ); ?></label>
					</th>
					<td>
						<input type="number" name="google_timezone_cache_duration" id="google_timezone_cache_duration"
						       ...
						       min="300" step="300">
						<p class="description">
							<?php _e( 'How long to cache results in seconds. Default is 86400 (24 hours).', 'arraypress-google-timezone' ); ?>
						</p>
					</td>
				</tr>
			</table>
			<?php submit_button(
				empty( get_option( 'google_timezone_api_key' ) )
					? __( 'Save Settings', 'arraypress-google-timezone' )
					: __( 'Update Settings', 'arraypress-google-timezone' ),
				'primary',
				'submit_api_key'
			); ?>
		</form>
